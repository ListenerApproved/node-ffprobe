...
module.exports = (function() {
	...
	function getCommandName(callback) {
		which('ffprobe', function (err) {
			if (!err) {
				return callback(null, 'ffprobe');
			}

			which('avprobe', function (err) {
				if (!err) {
					return callback(null, 'avprobe');
				}

				callback(new Error('Cannot find ffprobe/avprobe executable'))
			})
		});		
	}

	function doProbe(file, callback) {
		getCommandName(function(err, name) {
			if (err) {
				return callback(err);
			}

			var proc = spawn(name, ['-show_streams', '-show_format', '-loglevel', 'warning', file]),
					probeData = [],
					errData = [],
					exitCode = null,
					start = Date.now();
