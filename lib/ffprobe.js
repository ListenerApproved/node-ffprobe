...
module.exports = (function() {
	function parseBlock(block) {
		...
		lines.forEach(function(line) {
			var data = line.split('=');
			if(data && data.length === 2) block_object[data[0]] = data[1].trim();
		});
	...
	function parseStreams(text, callback) {
		...
		if(start === -1 || end === -1) {
			console.log("Missing STREAM block(s) - run FFProbe with the '-show_streams' option");
			return { streams: null };
		}
	...
	function parseFormat(text, callback) {
		...
		if(start === -1 || end === -1) {
			console.log("Missing FORMAT block - run FFProbe with the '-show_format' option");
			return { format: null }
		}
